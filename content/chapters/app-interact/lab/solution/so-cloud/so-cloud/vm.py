...
def start_qemu_for_vm(vm: VM):
    ...
    qemu_cmd = ['qemu-system-x86_64',
                '-enable-kvm',
                '-m', f'{mem_size_mb}',
                '-hda', f'/vm-disks/{vm.vm_disk.id}/disk.qcow2',
                '-net', f'nic,macaddr={vm.macaddr}',
                '-net', f'tap,ifname=tap{vm.tap_iface_idx},script=no',
                '-monitor', f'telnet::{vm.qemu_monitor_port},server,nowait',
                '-serial', f'telnet::{vm.qemu_serial_port},server,nowait',
                '-nographic']
...
def ubuntu_22_04_vm_prepare(vm: VM):
    ...
    e.sendline('echo ", +" | sfdisk --force -N 1 /dev/sda && partprobe && resize2fs -f /dev/sda1')
    ...
    e.expect_exact('root@ubuntu:~# ')
    e.sendline("sed -i 's/#PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config")

    e.expect_exact('root@ubuntu:~# ')
    e.sendline("sed -i 's/PasswordAuthentication no/PasswordAuthentication yes/' /etc/ssh/sshd_config")
    ...
    e.expect_exact('root@ubuntu:~# ')
    e.sendline(('cat > /etc/systemd/network/config.network <<EOF\n'
                '[Match]\n'
                'Name=ens3\n'
                '\n'
                '[Network]\n'
...
def vm_create(name: str, image: str, network_name: str,
              mem_size: int, disk_size: int):
    ...
    vm = VM(name, mem_size, vm_ip, net, tap_iface_idx,
            vm_disk, qemu_monitor_port, qemu_serial_port, -1, -1)

    vm.id = db.create_vm(name, vm_disk.id, vm.mem_size, vm.net.id,
                         vm.tap_iface_idx, vm.ip, vm.qemu_pid,
                         vm.qemu_monitor_port, vm.qemu_serial_port,
                         VM_STATE_RUNNING)

    try:
        if image == 'ubuntu_22.04':
            ubuntu_22_04_vm_prepare(vm)
...
def vm_get(id_: str):
    ...
    vm_net = network.Net(vm_networkid, network_name, network_bridge_interface_idx, network_ip, network_mask)

    vm = VM(vm_name, vm_mem_size, vm_ip, vm_net, vm_tap_interface_idx,
            vm_disk, vm_qemu_monitor_port, vm_qemu_serial_port, vm_qemu_pid, vm_state)
    vm.id = vm_id
