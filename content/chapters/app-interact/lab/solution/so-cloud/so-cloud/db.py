...
def create_disk(size: int, template_name: str):
    return simple_insert_query(('INSERT INTO disk (size, template_name) '
                                'VALUES (%s, %s)'),
                               (size, template_name))
...
def create_vm(name: str,
              ...
              state: int):
    return simple_insert_query(('INSERT INTO vm ('
                                'name, '
                                'disk_id, '
                                'mem_size, '
                                'network_id, '
                               ...
                               (name, disk_id, mem_size, network_id, tap_interface_idx,
                                ip, qemu_pid, qemu_monitor_port, qemu_serial_port, state))


def delete_vm(id: int):
...
def get_vm_info(id_: str):
    return simple_query(('SELECT v.id, v.name, v.disk_id, v.mem_size, v.network_id, v.tap_interface_idx, '
...
def update_vm_qemu_pid(id_: str, pid: int):
    simple_query('UPDATE vm SET qemu_pid = %s WHERE id = %s',
                 (pid, id_))

def update_vm_state(id_: str, state: int):
    simple_query('UPDATE vm SET state = %s WHERE id = %s',
                 (pid, id_))
...
def get_network_info(network_name: str) -> Tuple[str, str, str]:
    conn = mysql.connector.connect(
            host=host,
            user=user,
            password=password,
    ...
    cursor.execute('SELECT id, bridge_interface_idx, ip, mask FROM network WHERE name = %s',
                   (network_name,))
