...
def api_vm_create():
    ...
    try:
        error_str = ''
        vm_id = vm.vm_create(name, image, network_name, mem_size, disk_size)
        return jsonify({ 'status': 'ok', 'id': vm_id })
    except errors.VMAlreadyExistsException as ex:
...
def api_vm_info():
    ...
    app.logger.info(f'args {vm_info_args}')
    ...
    try:
        error_str = ''
        v = vm.vm_get(id_)
        return jsonify({ 'id': v.id, 'name': v.name, 'mem_size': v.mem_size,
                         'disk_size': v.vm_disk.size, 'os': v.vm_disk.template_name,
                         'network': v.net.name, 'ip': v.ip_str })
