...
{
    compiler_warn=$(< checker.out grep -v 'unused parameter' | grep -v 'unused variable' | grep -v "discards 'const'" | \
        grep -c '[0-9]\+:[0-9]\+: warning:')
    ...
    checkpatch_err=$(< checker.out grep 'total: [0-9]* errors' | grep -o '[0-9]* errors,' | cut -d ' ' -f 1 | paste -s -d '+' | bc)
    checkpatch_warn=$(< checker.out grep 'total: [0-9]* errors' | grep -o '[0-9]* warnings,' | cut -d ' ' -f 1 | paste -s -d '+' | bc)
...
{
    echo -e "\n\n### CHECKER\n\n"
    make check 2>&1 | sed 's/^Total:/Checker:/g' | tee checker.out

    echo -e "\n\n### LINTER\n\n"
    make lint 2>&1 | tee linter.out
